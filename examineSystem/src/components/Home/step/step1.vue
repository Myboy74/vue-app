<template>
        <div class="step1-content">
            ...
            <van-popup v-model="showClassify" position="bottom" :overlay="true">
                  <van-checkbox-group v-model="result" @change="toggleValue">
...
export default {
  created() {
    ...
    let types = this.$store.state.stepOneType; //分类
    let peoArr = this.$store.state.stepOneCon; //其他信息
    if (this.goBack) {
      this.classify = "";
      types.forEach(item => {
        this.result.push(item.classificationName);
  ...
  methods: {
    ...
    zuzhiType(id) {
      ...
      postAjax(
        "meetingClassInfication/getMeetingClassificationList",
        date
      ).then(res => {
